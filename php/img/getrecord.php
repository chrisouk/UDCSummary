<?php
    ...
    if ($subclasses)
    {
        $sql =  "select c.classmark_id, c.classmark_tag, c.broader_category, c.hierarchy_level, h.hierarchy_code, c.derived_from, f.description " .
                ...
                " left outer join language_fields f on f.classmark_id = (select classmark_id from classmarks where classmark_tag = c.derived_from) " .
...
//    $start=explode(' ',microtime());
    ...
    foreach($subclassarray as $record)
    {
        ...
        if ($record->derivedfrom != "")
        {
            ...
            $result .= "<tr>";
            $result .= "<td class=\"extag\">Derived from:</td><td class=\"exdesc\"><a href=\"#\" onclick=\"javascript:openrecord('". $record->derivedfrom . "')\">" . $record->derivedfrom . "</a> [see rule]</td>";
            $result .= "</tr>";   
            $result .= "</table></div>";
            $result .= "</div>";
        }
