...
//		
	...
	function dumpvarvalues()
	{
		...
		echo "seeinstructionsat = " . $this->seeinstructionsat . "<br>\n";
	}
...
//		echo "#Searchterm=" . $this->searchterm. "<br>\n";
		...
		$this->setformvar('usespecialchars', 	$this->usespecialchars);
   		...
   		$this->setformvar('seeinstructionsat', 	$this->seeinstructionsat);
	}
	...
	function setformvars(&$formstring, $oldvalues, $resetID = false)
	{
		...
		*/
		...
		$this->SetLanguageField('#seeinstructionsatcolor#', '#seeinstructionsat#', $formstring, $this->seeinstructionsat);
		...
		if (count($this->validation_errors) > 0)
		{
			if ($this->validation_errors[0] == "Success")
			{
				...
				if ($this->differences != "")	
				{
					$formstring = str_replace("#showdifferences#", "block", $formstring);
					$formstring = str_replace("#differences#", $this->differences, $formstring);
				}
				else
				{
					$formstring = str_replace("#showdifferences#", "none", $formstring);
					$formstring = str_replace("#differences#", $this->differences, $formstring);
				}
			}	
			else
...
//	  }
		...
		// Retrieve the parallel derivation details
		$sql =  "select c.classmark_tag, d.derived_from, f.language_id, f.description " .
				" from parallel_derivation d join classmarks c on d.derived_from = c.classmark_id " .
				" join language_fields f on d.derived_from = f.classmark_id and f.field_id = 1 and f.language_id in (1, " . $this->language_id . ") " .
				" where c.active = 'Y' order by f.language_id";
		//echo $sql . "<br>\n";
		...
		if ($resultcount > 0)
		{
			while(($row = @mysql_fetch_array($res, MYSQL_NUM)))
			{
				if ($row[2] == 1 || $row[2] == $this->language_id)
					$this->derivedfrom = $row[2] . '~' . specialchars($row[0]) . " " . specialchars($row[3]);
			}
			//echo "MFN is " . $this->mfn . "<br>\n";
		}
		else
		...
		$sql =  "select c.classmark_tag, d.derived_from, f.language_id, f.description " .
				" from parallel_derivation d join classmarks c on d.see_instructions_at = c.classmark_id " .
				" join language_fields f on d.see_instructions_at = f.classmark_id and f.field_id = 1 and f.language_id in (1, " . $this->language_id . ") " .
				" where c.active = 'Y' order by f.language_id";
		//echo $sql . "<br>\n";
		...
		if ($resultcount > 0)
		{
			...
			{
				if ($row[2] == 1 || $row[2] == $this->language_id)
					$this->seeinstructionsat = $row[2] . '~' . specialchars($row[0]) . " " . specialchars($row[3]);
			}
			//echo "MFN is " . $this->mfn . "<br>\n";
		}
		else
		...
		$sql = "select f.field_id, f.language_id, description from language_fields f, classmarks c where c.classmark_id = " . $this->mfn .
			   " and c.classmark_id = f.classmark_id and f.language_id in (1, " . $this->language_id . ") order by f.field_id, f.language_id"; 
		//echo $sql . "<br>\n";
		...
		$sql = "select r.notation, f.language_id, f.description from language_fields f, classmark_refs r, classmarks c where r.classmark_id = " . $this->mfn .
			   " and r.notation = c.classmark_tag and c.classmark_id = f.classmark_id and f.field_id = 1 and f.language_id in (1, " . $this->language_id . ") and c.active = 'Y' order by c.classmark_enc_tag";
		//echo $sql . "<br>\n";
		...
		$sql = "select e.field_type, e.tag, e.encoded_tag, c.classmark_tag, f.description, f.language_id " .
			   ...
			   "left outer join language_fields f on f.classmark_id = e.classmark_id and e.seq_no = f.seq_no and f.field_id = 2 where c.classmark_id = " . $this->mfn . " and f.language_id in (1, " . $this->language_id . ") order by e.encoded_tag";  
	   ...
	   //echo "Checking form vars<br>\n";
		...
		if ($this->CheckFormVars($errors))
		{
			if (isset($_SESSION['updates_allowed']) && $_SESSION['updates_allowed'] == "Y")
			{
				...
				//echo "fin<br>\n";
				$this->differences = $this->GetRecordDiffs($oldrecord, $sqlarray);
