...
class UDCForm
{
	...
	function EndTime($operation, $starttime)
	{
		...
		array_push($this->times, $operation . "<br>\n");
		array_push($this->times, "Executed in " .$totaltime. " seconds<br>\n");
	}	
...
//        echo "#Searchterm=" . $this->searchterm. "<br>\n";
	...
	function queryformvars($dsn)
	{
        ...
        if ($res)
        {
    		...
    		$this->EndTime("Classmark id selection", $starttime);
        ...
        if ($valid_notation == false)
        {
            ...
            if ($res)
            {
                ...
                $this->EndTime("Invalid notation error", $starttime);
                @mysql_free_result($res);                
            }
        ...
        if ($res)
        {
    		...
    		if ($resultcount > 0)
    		{
    			...
    			$this->EndTime("Hierarchy code fetch", $starttime);
    		}
		...
		if ($resultcount > 0)
		{
			...
			$this->EndTime("Broader category fetch", $starttime);
		}
		else
		{
		...
		if ($resultcount > 0)
		{
			...
			$this->EndTime("DerivedFrom fetch", $starttime);
		}
		else
		{
			//echo "No rows returned for broader<br>\n";
		...
		if ($resultcount > 0)
		{
			...
			$this->EndTime("Language field fetch", $starttime);
		}
		else
		{
			if (count($this->validation_errors) == 0)
		...
		$sql = "select r.notation, f.language_id, f.description from classmark_refs r join classmarks c on r.notation = c.classmark_tag and c.active = 'Y' " .
			   " join language_fields f on c.classmark_id = f.classmark_id and f.field_id = 1 where r.classmark_id = " . $this->mfn;
		...
		if ($resultcount > 0)
		{
			...
			$this->EndTime("Reference fetch", $starttime);
		}
		else
		{
		...
		if ($resultcount > 0)
		{
			...
			$this->EndTime("Example fetch", $starttime);
		}
		...
		$this->EndTime("Encoding examples", $starttime);
		...
		if ($resultcount > 0)
		{
			...
			$this->EndTime("Parallel division fetch", $starttime);
		}
		else
		{
			//echo "No rows returned<br>\n";
		...
		*/
		...
		if ($resultcount > 0)
		{
			...
			$this->EndTime("Special aux group fetch", $starttime);
		}
		else
		{
			//echo "No rows returned<br>\n";
		...
		if ($resultcount > 0)
		{
			...
			$this->EndTime("Other annotation fetch", $starttime);
		}
		else
		{
			//echo "No rows returned<br>\n";
		...
		if ($resultcount > 0)
		{
			...
			$this->EndTime("Other editorial note fetch", $starttime);
		}
		else
		{
			//echo "No rows returned<br>\n";
		...
		if ($resultcount > 0)
		{
			...
			$this->EndTime("Audit history fetch", $starttime);
		}
		else
		{
			//echo "No rows returned<br>\n";
		...
		if ($resultcount > 0)
		{
			...
			$this->EndTime("Last revision fetch", $starttime);
		}
		else
		{
			//echo "No rows returned<br>\n";
		...
		if ($resultcount > 0)
		{
			...
			$this->EndTime("Revision history fetch", $starttime);
		}
		...
		if ($resultcount > 0)
		{
			...
			$this->EndTime("Revision history field fetch", $starttime);
		}
		...
		if ($resultcount > 0)
		{
			...
			$this->EndTime("Editor comments", $starttime);
		}
		...
		if ($resultcount > 0)
		{
			...
			$this->EndTime("Keywords", $starttime);
		}
		...
		if ($resultcount > 0)
		{
			...
			$this->EndTime("Alpha index fetch", $starttime);
		}
		...
		if ($resultcount > 0)
		{
			...
			$this->EndTime("Changed fields fetch", $starttime);
		}
