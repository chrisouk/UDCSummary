...
$errorstring = "";
$newrefstring = "";
$tablestring = "<table class=\"reftable\" width=\"100%\" bgcolor=\"#dddddd\" border=\"0\" cellpadding=\"2\" cellspacing=\"1\"><tr><td width=\"15%\" align=\"left\">Notation</td><td width=\"80%\" align=\"left\">Description</td><td width=\"5%\">&nbsp;</td></tr>\n";
...
{		
	...
	$sql = "select f.description, c.active from language_fields f, classmarks c where c.classmark_id = f.classmark_id and field_id = 1 and f.language_id = 1 and c.classmark_tag = '" . 
            mysql_real_escape_string($parts[0], $dbc) . "'";
	...
	if ($resultcount > 0)
	{
        ...
        if($row[1] == 'N')
        {
            $errorstring = "*[" . specialchars($parts[0]) . "] is a cancelled notation*\n";
        }
        else
        {
    		$newrefstring .= htmlentities($parts[0], ENT_COMPAT) . "#" . htmlentities($row[0], ENT_COMPAT) . ";";		
    		$tablestring .= "<tr><td width=\"15%\" bgcolor=\"white\">" . specialchars($parts[0]) . "</td><td width=\"80%\" bgcolor=\"white\">" . specialchars($row[0]) . 
                            "</td><td width=\"5%\" bgcolor=\"white\"><a href=\"#refentry\" onMouseDown=\"javascript:deleteRef('". htmlentities($parts[0], ENT_COMPAT) . "');return true\">Delete</a></td></tr>\n";
        }
	}
	else
	{
